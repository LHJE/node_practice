...
app.post('/api/v1/footnotes', (request, response) => {
  ...
  for (let requiredParameter of ['paper', 'footnote']) {
    if (!paper[requiredParameter]) {
      return response
        .status(422)
        .send({ error: `Expected format: { paper: <Integer>, footnote: <String> }. You're missing a "${requiredParameter}" property.` });
    }
  }
...
app.get('/api/v1/papers/:id/footnotes', (request, response) => {
  database('footnotes').where('id', request.params.paper_id).select()
    .then(papers => {
      if (papers.length) {
